# Elementos básicos de HTML

## 📌 Sobre esta carpeta

Práctica de las etiquetas HTML más básicas de contenido: títulos, párrafos y demás elementos fundamentales.

Está dividida en dos subcarpetas:

- **[Ejercicios/](./Ejercicios)** — Prácticas que voy haciendo sobre lo visto en teoría.
- **[Teoria/](./Teoria)** — Apuntes y notas de los conceptos de este tema.

## 📂 Ejercicios

- **[Titulos](./Ejercicios/Titulos)** — Uso de las 6 etiquetas de encabezado (`<h1>` a `<h6>`) y su jerarquía visual.
- **[Parrafos](./Ejercicios/Parrafos)** — Uso de `<p>` para párrafos de texto y `<br>` para saltos de línea manuales dentro de un párrafo.
- **[Links](./Ejercicios/Links)** — Uso de `<a>` para enlaces externos e internos, y el atributo `target="_blank"`.
- **[imagenes](./Ejercicios/imagenes)** — Uso de `<img>` para insertar imágenes locales, con los atributos `src`, `alt` y `width`.

*(Esta lista se irá ampliando a medida que añada más ejercicios.)*

## 🔄 Estado

En progreso. Tema anterior: [`Introduccion/`](../Introduccion).